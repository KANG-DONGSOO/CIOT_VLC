...
READDATA:
	...
	SET		r30.t1				// [DEBUG] Check how long does it take to process all 16 received bits
	SET		r30.t2
	SET		r30.t2
	CLR		r30.t2
	
GET_CURRENT_BIT:
	SUB		r6, r11, r5				// * The 16 bit will be shifted (Counter - 1) times, to process 16 bits input from its MSB
	LSR		r7, r3, r6				// * r7 <- INPUT_16_bit >> N_shift_times
	AND		r7, r7, 0x1				// * r7 = r7 & 0x1 (to get only the first bit of r7 after shift)
	//MOV		r24, 5					// [DEBUG] Give delay to make the Oscilloscope output visible to human eye
	//QBBS	CUR_BIT_HIGH, r7.t0		// [DEBUG] IF current bit is SET THEN output HIGH in oscilloscope
	//CLR		r30.t2					// [DEBUG] ELSE output LOW in oscilloscope
	//QBA		CHECK_VALUE			// [DEBUG]

//CUR_BIT_HIGH:
//	SET		r30.t2					// [DEBUG] Outputing HIGH to Oscilloscope

//CHECK_VALUE: //[DEBUG] Used to output value of received bits to Oscilloscope to check correctness
//	SUB		r24, r24, 1
//	QBNE	CHECK_VALUE, r24, 0
//	QBEQ	READDATA, r5, 0
//	SUB		r5, r5, 1
//	QBA		GET_CURRENT_BIT
...
CLEAR:
	CLR		r30.t1				// [DEBUG] Check how long does it take to process all 16 received bits
	QBA READDATA
...
SFD:
	...
	ADD		r16, r16, r13						// * recv_sfd += cur_decoded_value
	QBEQ	COMPARE_WITH_SFD, r15, 0			// * IF cnt_symbol == 0 [already received 8 bits] THEN compare received 8 bits with SFD value
	SUB		r15, r15, 1							// * cnt_symbol--
	MOV		r4, 1								// * move state to OOK Detection first step
	QBA		AFTER_BIT_PROCESSED					// * finished processing bit
	
COMPARE_WITH_SFD:
	QBEQ	TO_LENGTH, r16, r17					// * IF recv_sfd == sfd_value THEN proceed to length detection on next bit
	MOV		r15, 7								// * Reset the first and second variable of SFD detection
	MOV		r16, 0								// *
	MOV		r4, 0								// * program_state <- 0 [Back to beginning, Preamble Detection]
	QBA		AFTER_BIT_PROCESSED					// * finished processing bit
...
TO_LENGTH:
	...
	MOV		r4, 1								// * move state to OOK Detection first step
	QBA		AFTER_BIT_PROCESSED					// * finished processing bit
	//QBA		END									// [DEBUG] HALT PROGRAM WHEN SFD IS DETECTED TO ENSURE SFD DETECTION WORKS CORRECTLY
...
PROCESS_LENGTH:
	...
	QBGE	LENGTH_INVALID, r16, 0				// * IF payload_len <= 0 THEN length is invalid
	QBLT	LENGTH_INVALID, r16, r22			// * IF payload_len > mtu THEN length is invalid
	//Write the 2 bytes of length to memory
	//[TODO]Get current offset for writing the packet
	MOV		r20, 0								// [DEBUG] Give initial offset as 0
	
	SBBO	r16, r0, r20, 2						// * store payload_len value to memory
	ADD		r20, r20, 2							// * curr_offset += 2 [size of payload length is 2 byte]
...
BODY:
	...
	QBEQ	PROCESS_PACKET_BYTE, r15, 0				// * IF cnt_symbol == 0 [already received 8 bits] THEN process the value of payload length
	//ADD		r15, r15, 1
	SUB		r15, r15, 1							// * cnt_symbol--
	MOV		r4, 1								// * move state to OOK Detection first step
	QBA		AFTER_BIT_PROCESSED					// * finished processing bit
