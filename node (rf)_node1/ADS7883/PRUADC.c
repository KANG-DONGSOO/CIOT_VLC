...
int main (void)
{
   ...
   unsigned int ookData[3];
   ookData[0] = FREQ_12_5MHz;
   ookData[1] = readFileValue(MMAP1_LOC "addr");
   ookData[2] = max_offset_value;
   printf("The PRU clock state is set as period: %d (0x%x) and state: %d\n", ookData[0], ookData[0], ookData[1]);
   ...
   unsigned int spiData[4];
   spiData[0] = 0x00000000;
   spiData[1] = readFileValue(MMAP1_LOC "addr");
// spiData[2] = readFileValue(MMAP1_LOC "size"); //NUMBER OF SAMPLE BYTES BASED ON THE SIZE OF ALLOCATED MEMORY
   spiData[2] = max_offset_value; //NUMBER OF SAMPLE BYTES BASED ON THE SIZE OF SHARED RAM AMONG PRUs
   spiData[3] = FREQ_12_5MHz;
   printf("Sending the SPI Control Data: 0x%x\n", spiData[0]);
   printf("The DDR External Memory pool has location: 0x%x and size: 0x%x bytes\n", spiData[1], readFileValue(MMAP1_LOC "size"));
   ...
   prussdrv_pru_write_memory(PRUSS0_PRU1_DATARAM, 0, ookData, 12); // sample clock
   ...
   prussdrv_exec_program (CLK_PRU_NUM, "./PRUOOK.bin");
