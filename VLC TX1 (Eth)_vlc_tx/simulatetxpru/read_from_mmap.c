...
uint16_t consume_packet(uint16_t read_idx){
	uint32_t offset = 0;
	uint16_t self_id = 0,dest_id=0,length = 0,seq_number1 = 0, seq_number2 = 0;
...
//    	*((uint16_t *) read_pointer_addr) = (uint16_t) 40;
   ...
   // for(;;){
    ...
    for(;;){
//    	do{
    		read_idx = *((uint16_t *) data_pointer_addr + read_idx_offset);
			write_idx = *((uint16_t *) data_pointer_addr);
			printf("Read idx: %d\nWRite Idx: %d\n\n",read_idx,write_idx);
//    	}while(read_idx == write_idx);
//
//		read_idx = consume_packet(read_idx);
//		if(read_idx == -1){
//			break;
//		}
...
//			read_idx = 4;
//		}
//		printf("Read idx after: %d\n",read_idx);
//
//		*((uint16_t *) data_pointer_addr + read_idx_offset) = (uint16_t) read_idx;
    }
