...
#define DAC_PRU_NUM	   0   // using PRU0 for the ADC capture
...
int main (void)
{
   ...
   unsigned int dac_data[1];
   dac_data[0] = FREQ_5MHz;
   ...
   prussdrv_pru_write_memory(PRUSS0_PRU0_DATARAM, 0, dac_data, 4);  // spi code

   // Map the PRU's interrupts
   prussdrv_pruintc_init(&pruss_intc_initdata);

   // Load and execute the PRU program on the PRU
   prussdrv_exec_program (DAC_PRU_NUM, "./PRUDAC.bin");
