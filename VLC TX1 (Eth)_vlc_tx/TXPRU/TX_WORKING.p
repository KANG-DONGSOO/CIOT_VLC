...
START:
	...
	MOV		r13, 1					// * r13 = OOK Encoded value of HIGH (LOW - HIGH) 
	MOV		r14, 2					// * r14 = OOK Encoded value of LOW (HIGH - LOW)
...
SHIFTLOOP:
	...
	//CLR		r30.t1					// [DEBUG] Oscilloscope output - not output period
...
LOADGPOSH0:
	...
	MOV		r30.b1, 0x00			// - GPO SH0.b1 <- current_value.b1
	MOV		r30.b0, 0x00			// - GPO SH0.b0 <- current_value.b0
...
LOADGPOSH1:
	...
	MOV		r30.b1, 0xFF			// - GPO SH1.b1 <- current_value.b1
	MOV		r30.b0, 0xFF			// - GPO SH1.b0 <- current_value.b0
...
PREAMBLE:
	LSL		r3, r3, 1						// - current_value = current_value<< 1 [Shift left the current value for 1 bit]
...
ENCODEHIGH:
	LSL		r3, r3, 2						// - current_value = current_value<<2 [Shift left 2 bits for next encoded value]
	ADD		r3, r3, r13						// - current_value += HIGH_VALUE (LOW HIGH)
	ADD		r1, r1, 2						// - transmitted_bit_counter += 2 [2 bits of encoded value]
	QBA		AFTERGETVALUE
	
ENCODELOW:
	LSL		r3, r3, 2						// - current_value = current_value<<2 [Shift left 2 bits for next encoded value]
	ADD		r3, r3, r14						// - current_value += HIGH_VALUE (LOW HIGH)
	ADD		r1, r1, 2						// - transmitted_bit_counter += 2 [2 bits of encoded value]
	QBA		AFTERGETVALUE
...
AFTERGETVALUE:								
	...
	//QBEQ	END, r19, 35					// [DEBUG] used to execute GETVALUE function with desired number of executions [Validating result of each step of encoding]
	RET