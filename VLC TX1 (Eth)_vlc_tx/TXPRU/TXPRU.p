...
START:
	...
	MOV		r9, 0					// * r9 = read_idx [Offset value to read from memory]
	...
	MOV		r23, ADDR_LIMIT
	MOV		r24, BUFFER_LIMIT		// * r24 = Buffer Limit [The size of circular buffer that is implemented]
	MOV		r25, 0					// * r25 = write_idx
	MOV		r26, 0					// * r26 = is_packet_finish [Flag to notify program whether a packet is successfully transmitted]
	MOV		r27, 0					// * r27 = is_state_reset
...
RESET_STATE_TO_PREAMBLE:
	...
	SET		r27.t0							// - is_state_reset <- true

CHECK_INDEX:
	LBBO	r25, r0, 0, 2						// - write_idx <- read from PRU memory
	LBBO	r9, r0, 2, 2						// - read_idx <- read from PRU memory
	QBNE	RESET_IS_PACKET_DONE, r9, r25					// - IF write_idx != read_idx THEN go to shift loop
	QBBC	RESET_STATE_TO_PREAMBLE, r27.t0 	// - IF is_state_reset == false THEN reset the state to preamble
	QBA		CHECK_INDEX

RESET_IS_PACKET_DONE:
	CLR		r27.t0					// - is_state_reset <- false
	CLR		r26.t0					// - is_packet_finish <- false
...
LOADGPOSH0:
	...
	QBBS	CHECK_INDEX, r26.t0		// - IF is_packet_finish == true THEN check write and read index to determine buffer empty or not
...
PACKET_DONE:
	...
	MOV		r2, 4									// - curr_state = 0 (PACKET_END)
	QBGT	INIT_NEXT_PREAMBLE, r9, r24 	// - IF read_idx <= BUFFER_LIMIT THEN skip to initiation of next preamble variables
	MOV		r9, 4
	
	
INIT_NEXT_PREAMBLE:	
	SBBO	r9, r0, 2, 2					// - store the current read_idx to PRU memory
	SET		r26.t0							// - is_packet_finish = TRUE
	//Initialize variables for next preamble detection
	MOV		r7, 3							// - cnt_symbol = 7
	MOV		r18, 0							// - cnt_preamble = 0
	QBA		ENCODEBIT
